...
| **Layout** | Left category rail when `category` (or **Category field key**) is in `Fields`. Toolbar: search, facet pills (optional), view toggle (grid / list). |
| **Card view** | Responsive grid (1 / 2 / 3 columns). Badge for facet field; title from `name` (or first field); description fields line-clamped to 2 lines; price-like fields emphasized. |
